...
doGet = do a <- doSStackPop ++ raiseError ("doGet: empty sstack")
        ...
        where value a = do heap <- getHeap
                           case heapLookup heap a of
                            ...
                            Just (NConstr tagTrue  []) -> return tagTrue
                            Just (NConstr tagFalse []) -> return tagFalse
                            Just (NInd a')             -> value a'
...
iStats s
   = iConcat [
       ...
       iStr "Maximum sstack length = ", iNum (statGetSStacks (gmStats s)),iNewline,
       iStr "Maximum vstack length = ", iNum (statGetVStacks (gmStats s)),iNewline,
       iStr "Maximum dump length = ", iNum (statGetDumps (gmStats s)),
