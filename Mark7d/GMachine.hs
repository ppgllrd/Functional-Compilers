...
doCond :: GMCode -> GMCode -> Monadic ()
doCond i1 i2 = do BasicInt n <- doVStackPop ++ raiseError ("doCond: vstack is empty or integer not found")
...
doPrint = do a <- doSStackPop ++ raiseError ("doPrint: empty sstack")
             ...
             case heapLookup heap a of
               ...
               Just (NAp _ _)      -> do i <- getCode
                                         putCode (codeFromList [PrintStr "Aplication"]++ i)                          
...
doMkbool :: Monadic ()
doMkbool = do BasicInt x <- doVStackPop ++ raiseError ("doMkbool: vstack is empty or integer not found")
              a <- doHeapAlloc (NConstr x [])
              doSStackPush a

doMkint :: Monadic ()
doMkint = do BasicInt x <- doVStackPop ++ raiseError ("doMkint: vstack is empty or integer not found")
             a <- doHeapAlloc (NInt x)
             doSStackPush a

doMkchar :: Monadic ()
doMkchar = do BasicChar x <- doVStackPop ++ raiseError ("doMkchar: vstack is empty or char not found")
...
doGet = do a <- doSStackPop ++ raiseError ("doGet: empty sstack")
        ...
        where value a = do heap <- getHeap
                           case heapLookup heap a of
                            ...
                            Just (NConstr tagTrue  []) -> return (BasicInt tagTrue)
                            Just (NConstr tagFalse []) -> return (BasicInt tagFalse)
                            Just (NInd a')             -> value a'
...
primitive1 op = do BasicInt n <- doVStackPop ++ raiseError ("primitive1: empty vstack or integer not found")
                   doVStackPush (BasicInt (op n))

primitive2 :: (Int->Int->Int) -> Monadic ()
primitive2 op = do BasicInt n0 <- doVStackPop ++ raiseError ("primitive2: empty vstack or integer not found")
                   BasicInt n1 <- doVStackPop ++ raiseError ("primitive2: empty vstack or integer not found")
...
charToInt op = do BasicChar c <- doVStackPop ++ raiseError ("charToInt: empty vstack or char not found")
                  doVStackPush (BasicInt (op c))
                  
intToChar :: (Int->Char) -> Monadic ()
intToChar op = do BasicInt n <- doVStackPop ++ raiseError ("intToChar: empty vstack or int not found")
