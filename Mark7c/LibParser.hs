...
instance MonadPlus (Parser s) where
  ...
  mzero = do
            ...
            raiseException MkError { errorMsg   = "Parse error",
                                     ...
                                     errorInput = i   }
     where 
      raiseException :: Error -> Parser s a
      raiseException err  = MkPM (\s -> Left3 err)

  -- mplus :: Parser s a -> Parser s a -> Parser s a 
  MkPM st1  `mplus`  MkPM st2  = 
    MkPM (\s -> case st1 s of
...
many1 p      = do x  <- p 
                  xs <- do {many1 p `mplus` return []}
                  return (x:xs)


many         :: Parser s a -> Parser s [a]
many p        = force (many1 p `mplus` return [])
...
chainl p op v      = (p `chainl1` op) `mplus` return v


chainl1           :: Parser s a -> Parser s (a -> a -> a) -> Parser s a
p `chainl1` op     = do {x <- p; rest x}
                     where
                        rest x =  mplus (do {f <- op; y <- p; rest (f x y)})
                                        (return x)


chainr            :: Parser s a -> Parser s (a -> a -> a) -> a -> Parser s a
chainr p op v      = (p `chainr1` op) `mplus` return v
...
ops xs             = foldr1 mplus [do {p; return op} | (p,op) <- xs]
...
pStr               :: String -> Parser s String
pStr ""             = return ""
pStr (x:xs)         = do {pChar x; pStr xs; return (x:xs)}
...
pInt               :: Parser s Int
pInt                = do {pChar '-'; n <- pNat; return (-n)} `mplus` pNat
