...
instance MonadPlus Program where
  ...
  -- mplus :: Program a -> Program a -> Program a
  MkProgram sc1 `mplus` MkProgram sc2 = MkProgram (sc1++sc2)


data Expr a
  =  EVar Name                     -- Variables
   | EInt Int                      -- Integers
   | EChar Char                    -- Characters
...
isAtomicExpr (EInt n)  = True
isAtomicExpr (EChar c) = True
...
pprExpr (EAp (EAp (EVar op) e1) e2) p 
		      ...
		      | op == tkAdd   = pprOper e1 op e2 (p>4) 5 4
		      | op == tkSub   = pprOper e1 op e2 (p>4) 5 5
        	      | op == tkMul   = pprOper e1 op e2 (p>5) 6 5
...
pprAlter (MkAlter (tag,args,expr)) = 
   iConcat [iStr tkLT, iInt tag, iStr tkGT, iStr " ",
...
pprExpr (EInt n)             = iStr (show n)
pprExpr (EConstr tag arity)  = iConcat [iStr tkPack, iStr " ", iStr tkOCB, iStr " ", iInt tag, iStr tkComma, iInt arity, iStr tkCCB]
