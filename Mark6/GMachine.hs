           ...
           where newState (NNum _)      = do a <- doStackTop
                                             ...
                                             case dumpPop dump of
                                                    ...
                                                    Just (d, i', s') -> do putCode i'
                                                                           ...
                                                                           doStackSet newStack
                                                                           putDump d
                 newState (NConstr _ _) = do a <- doStackTop
                                             dump <- getDump
                                             case dumpPop dump of
                                                    Nothing -> raiseError ("doUnwing: empty dump")
                                                    Just (d, i', s') -> do putCode i'
                 ...
                 newState (NGlobal n c) = do stack <- getStack
                                             ...
                                             if k < n then do ak <- doStackSelect k
                                                              dump <- getDump
                                                              case dumpPop dump of
                                                                     Nothing -> raiseError ("doUnwing: empty dump")
                                                                     Just (d, i, s) -> do putCode i
                                                                                          let newStack = stackPush s ak
                                                                                          doStackSet newStack
                                                                                          putDump d
                                                      else do putCode c
                                                              rearrangeStack n
...
doPack t n = do as <- accumulate (replicate n doStackPop) ++ raiseError ("doPack: unsaturated constructor")
                a <- doHeapAlloc (NConstr t as)
                doStackPush a
                

doCasejump :: [(Int,GMCode)] -> Monadic ()
doCasejump alts = do a <- doStackTop ++ raiseError ("doCasejump: NConstr node expected on stack")
                     heap <- getHeap
                     case heapLookup heap a of
                       Just (NConstr t ss) -> case [i | (tag,i)<-alts, tag==t] of
                                                ...
                                                i':_ -> do i <- getCode
                                                           putCode (i' ++ i)
                       Just _              -> raiseError ("doCasejump: NConstr node expected on stack")
                       Nothing             -> raiseError ("doCasejump: can`t find address "++showAddr a++" in heap")


doSplit :: Int -> Monadic ()
doSplit n = do a <- doStackPop ++ raiseError ("doSplit: empty stack")
               heap <- getHeap
               case heapLookup heap a of
                 Just (NConstr t as) -> sequence (map doStackPush (reverse as))
                 Just _              -> raiseError ("doSplit: NConstr node expected on stack")
                 Nothing             -> raiseError ("doSplit: can`t find address "++showAddr a++" in heap")


doPrint :: Monadic ()
doPrint = do a <- doStackPop ++ raiseError ("doPrint: empty stack")
             heap <-  getHeap
             case heapLookup heap a of
               ...
               Just (NConstr t []) -> do let iBegin = codeFromList [PrintStr ("C"++show t)]
                                         i <- getCode
                                         putCode (iBegin ++ i)               
               Just (NConstr t as) -> do sequence (map doStackPush (reverse as))
                                         i <- getCode
               ...
               Nothing             -> raiseError ("doPrint: NNum or NConstr expected on stack")
